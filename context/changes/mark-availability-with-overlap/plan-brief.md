...
| Data model                        | One row per `(group_id, user_id, slot_date)`; `slot_hour` is the **start hour** and availability lasts to end-of-day. Corrected mid-Phase-3 from a slot-per-hour design — see plan.md Critical Implementation Details. | PRD FR-006 ("day + start-hour slot") + §Business Logic intent surfaced during Phase 3 manual testing; cumulative overlap is the wedge.                                       | Plan + Implement |
